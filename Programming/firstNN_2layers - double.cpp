...
int		forward_pro(){
	...
	for (i = 0; i<HIDDEN_02; i++){
		for (j = 0; j<HIDDEN_01; j++){
			temp += weight_02[j][i] * sigmoid_output_01[j];
		}
	...
	for (i = 0; i<10; i++){
		for (j = 0; j<HIDDEN_02; j++){
			temp += weight_03[j][i] * sigmoid_output_02[j];
		}
...
//		sig = 1/(1-exp(-weighted_sum_03[i]));
	...
	for (i = 0; i<HIDDEN_02; i++){
		temp = 0;
		for (j = 0; j<10; j++){
			temp += delta_output[j] * weight_03[i][j];
		}
	...
	for (i = 0; i<HIDDEN_01; i++){
		temp = 0;
		for (j = 0; j<HIDDEN_02; j++){
			temp += delta_hidden_02[j] * weight_02[i][j];
		}
