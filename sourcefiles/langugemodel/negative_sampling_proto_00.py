...
# @Last modified time: 2018-11-14T15:46:04+09:00
# @Copyright: JayY

# negative sampling proto type #1
# ===============================================
...
with tf.Session() as session:
    ...
    if os.path.isdir('./nce_korean_wiki/'):
        for path, dir, files in os.walk('./nce_korean_wiki/'):
            for filename in files:
                ...
                if ext == '.meta':
                    saver.restore(session, tf.train.latest_checkpoint('nce_korean_wiki'))
    else:
    ...
    for step in range(epoch):
        ...
        for filename1 in filename_list:
            ...
            saver.save(session, './nce_korean_wiki/korean_wiki_model_nce', global_step=step)
