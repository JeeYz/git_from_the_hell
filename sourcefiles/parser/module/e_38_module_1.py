...
# @Last modified time: 2019-03-08T14:26:38+09:00
# @License: J.Y. JeeYz
# @Copyright: J.Y. JeeYz
...
def make_dependency_tree(given):
    head = word_node()
    node_list = list()
    for j,i in enumerate(reversed(given)):
        if j == 0:
            head.index = i[0]
            head.word = i[2]
...
def make_one_train_data(stack, buffer, word_element, root):
    # print(stack)
    # print(buffer)
    one_data = list()
...
def print_tree(curr, node_list):
    if curr.childlist == []:
        return
    else:
        for i in curr.childlist:
            print_tree(i, node_list)
            # print(i.index, '\t', i.word, '\t', i.childlist, '\t', i.parent)
            # node_list.append(i)
